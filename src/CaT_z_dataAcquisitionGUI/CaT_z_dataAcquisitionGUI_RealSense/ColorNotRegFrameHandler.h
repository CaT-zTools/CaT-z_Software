...
class ColorNotRegFrameHandler : public facegram::cam::FrameHandler {
  private:
    const unsigned int width, height;
    const std::string outputFile;
    cv::VideoWriter vwNotReg;
    std::vector<int> _compression_params;

    unsigned long int frameCount = 0;

  public:
    ColorNotRegFrameHandler(std::string outputFile, unsigned int width, unsigned int height, std::vector<int> compression_params);
    ~ColorNotRegFrameHandler();
